...
// GET /api/analytics/cache/dean/:dean_id - Get cached analytics data for dean
router.get('/cache/dean/:dean_id', async (req, res) => {
  const { dean_id } = req.params;
  ...
  try {
    ...
    const query = `
      SELECT cache_id, cache_type, data_json, last_updated
      FROM dashboards_data_cache
      WHERE cache_type = 'dean_analytics' AND user_id = $1
      ORDER BY last_updated DESC
      LIMIT 1
    `;
    
    const result = await client.query(query, [dean_id]);
...
// POST /api/analytics/cache/dean/:dean_id - Store analytics data for dean
router.post('/cache/dean/:dean_id', async (req, res) => {
  const { dean_id } = req.params;
  const { analytics_data } = req.body;
  ...
  try {
    ...
    // First, delete any existing cache for this dean
    const deleteQuery = `
      DELETE FROM dashboards_data_cache
      WHERE cache_type = 'dean_analytics' AND user_id = $1
    `;
    
    await client.query(deleteQuery, [dean_id]);
    
    // Insert new cache data
    const insertQuery = `
      INSERT INTO dashboards_data_cache (cache_type, user_id, data_json, last_updated)
      VALUES ('dean_analytics', $1, $2, NOW())
      RETURNING cache_id, last_updated
    `;
    
    const result = await client.query(insertQuery, [dean_id, analytics_data]);
...
// DELETE /api/analytics/cache/dean/:dean_id - Clear cached analytics data for dean
router.delete('/cache/dean/:dean_id', async (req, res) => {
  const { dean_id } = req.params;
...
// GET /api/analytics/cache/program-chair/:program_chair_id - Get cached analytics data for program chair
router.get('/cache/program-chair/:program_chair_id', async (req, res) => {
  const { program_chair_id } = req.params;
  ...
  try {
    ...
    const query = `
      SELECT cache_id, cache_type, data_json, last_updated
      FROM dashboards_data_cache
      WHERE cache_type = 'program_chair_analytics' AND user_id = $1
      ORDER BY last_updated DESC
      LIMIT 1
    `;
    
    const result = await client.query(query, [program_chair_id]);
...
// POST /api/analytics/cache/program-chair/:program_chair_id - Store analytics data for program chair
router.post('/cache/program-chair/:program_chair_id', async (req, res) => {
  const { program_chair_id } = req.params;
  const { analytics_data } = req.body;
  ...
  try {
    ...
    // First, delete any existing cache for this program chair
    const deleteQuery = `
      DELETE FROM dashboards_data_cache
      WHERE cache_type = 'program_chair_analytics' AND user_id = $1
    `;
    
    await client.query(deleteQuery, [program_chair_id]);
    
    // Insert new cache data
    const insertQuery = `
      INSERT INTO dashboards_data_cache (cache_type, user_id, data_json, last_updated)
      VALUES ('program_chair_analytics', $1, $2, NOW())
      RETURNING cache_id, last_updated
    `;
    
    const result = await client.query(insertQuery, [program_chair_id, analytics_data]);
...
// DELETE /api/analytics/cache/program-chair/:program_chair_id - Clear cached analytics data for program chair
router.delete('/cache/program-chair/:program_chair_id', async (req, res) => {
  const { program_chair_id } = req.params;
